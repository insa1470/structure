...
PROMPT_CHART2 = """這是一張企查查或類似企業查詢 App 的「集團公司列表 / 子公司列表」手機長截圖。畫面通常由很多公司卡片自上而下排列，每一張卡片代表一家公司。

你的任務不是總結，而是逐列抽取公司資料。請嚴格依照畫面從上到下，一家公司一家公司處理，不能跳列，不能合併相鄰兩家公司。

請遵守以下規則：

一、只抽「公司卡片」，不要抽頁面雜訊
- 忽略頂部導覽列、分頁按鈕、搜尋列、篩選條件、底部 logo、浮水印、廣告、App 提示文字、icon。
- 只處理真正的公司列表卡片。
...
- ed: 成立日期
- cs: 公司狀態
- sl: 第幾級子公司，例如「二級子公司」「三級子公司」
- ac: 實控人總持股，例如「99.5%」

三、欄位定位規則
- 公司名稱通常在每張卡片最上方，字最大，必須完整抄寫，不可縮寫。
- cs 通常是綠色小標籤，例如「存續」「在業」。
- lr、rc、ed 通常在公司名稱下方同一區塊，可能分散在兩行。
- sl 通常在卡片右側的小框標籤，例如「二級子公司」。
- ac 通常出現在較下方，例如「實控人總持股 99.5%」。

四、抽取原則
- 只輸出圖片中實際可見的文字，禁止猜測。
- 看不清楚就填 null，不要編造。
- 允許某些欄位缺失，但公司名稱只要能辨識，就要保留該筆公司。
- 不要因為某一欄看不清楚，就整列跳過。
- 同一家公司只輸出一次。
- 若一張卡片的狀態不是「存續 / 在業 / 開業 / 仍註冊」等在營狀態，仍可先抽出，之後由程式端過濾；但 cs 欄位必須忠實抄寫。

五、避免常見錯誤
- 不要把右側「二級子公司 / 三級子公司」誤抄成公司名稱的一部分。
- 不要把上一家公司的持股比例接到下一家公司。
- 不要把「法定代表人」「註冊資本」「成立日期」這些欄位標題當成值。
- 不要把頂部數字、分類 tab、搜尋文字框內容、底部企查查 logo 當成公司資料。
- 如果某家公司只有公司名與部分欄位可見，也要輸出該公司。

六、輸出格式
只輸出 JSON array，不要有任何說明文字、markdown 或額外註解。

格式如下：
[{"c":"公司全名","lr":"法人代表或null","rc":"註冊資本如5000萬元或null","ed":"成立日期如2014-12-03或null","cs":"狀態如存續或null","sl":"如二級子公司或null","ac":"如99.5%或null"}]

七、輸出前自我檢查
- 每個物件都必須有 c, lr, rc, ed, cs, sl, ac 這 7 個 key。
- 每個值不是字串就是 null。
- ed 若有值，格式必須是 YYYY-MM-DD。
- 百分比若有值，保留 %。
- 不要漏掉中間任何一張公司卡片。
- 不要輸出空白公司名。
- 不要輸出重複公司。
