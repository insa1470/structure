...
PROMPT_CHART2_STAGE1 = """這是一張企查查或類似企業查詢 App 的「集團公司列表 / 子公司列表」手機長截圖。畫面通常由很多公司卡片自上而下排列，每一張卡片代表一家公司。

你的任務是先做第一階段：只抽公司清單骨架，不抽細節。
...
- 每張公司卡片只抽這 3 個欄位：
  - c: 公司全名
  - cs: 公司狀態，例如存續、在業、註銷
  - sl: 第幾級子公司，例如二級子公司、三級子公司
- 公司名稱必須完整抄寫，不可縮寫。
- 看不清楚就填 null，不要編造。
- 不要把右側「二級子公司 / 三級子公司」誤抄進公司名稱。
- 不要漏掉中間任何一張公司卡片。
- 不要輸出重複公司。

只輸出 JSON array，不要說明文字或 markdown：
[{"c":"公司全名","cs":"狀態如存續或null","sl":"如二級子公司或null"}]
"""

PROMPT_CHART2_STAGE2 = """這是一張企查查或類似企業查詢 App 的「集團公司列表 / 子公司列表」手機長截圖。畫面通常由很多公司卡片自上而下排列，每一張卡片代表一家公司。

你的任務是第二階段：只補公司詳細欄位。請依照畫面逐列辨識，但重點是盡量補出每家公司的細節，不要求每欄都一定成功。
...
只輸出 JSON array，不要說明文字或 markdown：
[{"c":"公司全名","lr":"法人代表或null","rc":"註冊資本如5000萬元或null","ed":"成立日期如2014-12-03或null","ac":"如99.5%或null"}]
"""
...
def analyze_chart2(image_path: Path) -> list[dict]:
    ...
    for base_row in stage1_rows:
        if not _is_active_company_status(base_row.get("company_status")):
            continue
        ...
        result.append({
            ...
            "company_status": base_row.get("company_status"),
            "subsidiary_level_label": base_row.get("subsidiary_level_label"),
            "actual_controller_share": detail_match.get("actual_controller_share") if detail_match else None,
            "uncertain": False,
        })
